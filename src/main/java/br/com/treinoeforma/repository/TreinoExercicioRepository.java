...
public interface TreinoExercicioRepository extends JpaRepository<TreinoExercicio,Long> {
	...
	@Query("SELECT NEW br.com.treinoeforma.model.TreinoExercicioValuesObject(te.treino, te.exercicio, MAX(te.titulo.id)) FROM TreinoExercicio te " + 
			"WHERE te.treino.id = :codigoTreino GROUP BY te.treino")
	public List<TreinoExercicioValuesObject> buscaUltimoTituloTreino(@Param("codigoTreino") Long codigoTreino);
